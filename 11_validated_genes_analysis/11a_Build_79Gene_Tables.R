# =============================================================================
# Build Two Tables for Validated GmJAG1 Targets
# 1. Supplementary Table: All validated genes with corrected functional categories
...
# TableS30 already has Cor_LW_Ratio, Module_Color, kME — only join missing cols
merged <- targets %>%
  left_join(pheno %>% select(GeneID, Cor_LW_Ratio, Pval_LW_Ratio),
            by = "GeneID", suffix = c("", ".pheno")) %>%
  left_join(hub %>% select(GeneID, Module_Color, kME),
            by = "GeneID", suffix = c("", ".hub"))
...
category_map <- tribble(
  ...
  # --- New in Phase3 (5 genes) ---
  "Glyma.18G273600",    "Transcription/Chromatin",  "AGAMOUS-LIKE 8 (AGL8/FUL); MADS-box TF, meristem identity",
  "Glyma.03G173300",    "Transcription/Chromatin",  "ZAT11; C2H2 zinc finger TF, stress/metal response",
  "Glyma.03G255900",    "Transport",                "CMP-sialic acid transporter; nucleotide-sugar transport",
  "Glyma.07G022602",    "Chloroplast",              "LPA3; low PSII accumulation 3, thylakoid biogenesis",
  "Glyma.19G020700",    "Metabolism",               "Aldose 1-epimerase; carbohydrate metabolism",
  # --- No annotation ---
...
# Check for missing and assign defaults
missing <- merged %>% filter(is.na(Functional_Category))
if (nrow(missing) > 0) {
  cat("NOTE:", nrow(missing), "genes without manual category (new in Phase3):\n")
  print(missing$GeneID)
  # Assign based on annotation if available
  merged <- merged %>%
    mutate(
      Functional_Category = ifelse(is.na(Functional_Category), "Uncategorized (new)", Functional_Category),
      Short_Function = ifelse(is.na(Short_Function),
                              ifelse(!is.na(Arabi_Defline), Arabi_Defline, "No annotation"),
                              Short_Function)
    )
}

# =============================================================================
# 4. SUPPLEMENTARY TABLE: ALL VALIDATED GENES
# =============================================================================
...
supp_table <- merged %>%
  mutate(
    Arabi_Short = sapply(Arabi_Defline, parse_defline)
  ) %>%
  select(
    GeneID,
    Arabidopsis_Ortholog = Arabi_Name,
    Functional_Category,
    Short_Function,
    DE_Tier,
    logFC = Mean_logFC,
...
write_csv(supp_table, file.path(out_dir, "TableS_Validated_Genes_AllTargets.csv"))
